...
# ── One-time buyer cleanup migration (2026-06-04) ───────────────────────────
# Folds outdated buyer strings in sales.json into their current identities so
# the sales-by-buyer analytics shows only active buyers. The actions are
# hardcoded (one-time); GET previews exactly what each matches against live
# data, POST applies. Idempotent: once a source is renamed/deleted it no longer
# matches, so a re-run is a no-op.
_BUYER_MIGRATIONS = [
    {"action": "rename",   "source": "SOMA (Wholesale)", "target": "SOMA (QBO)"},
    {"action": "relocate", "source": "Natures Emporium (Vaughan)",
     "parent": "Natures Emporium", "location": "Maple (Vaughan)"},
    {"action": "delete",   "source": "Inventory Adjustment (App Dev)"},
]


def _bm_norm(s):
    """Apostrophe/case-insensitive key for matching buyer strings (mirrors
    _buyer_resolver's normalisation)."""
    return (s or "").lower().replace("’", "").replace("'", "").strip()


@app.route("/admin/buyer-cleanup")
@login_required
def buyer_cleanup_page():
    """Render the one-time buyer-cleanup migration tool."""
    return render_template("buyer_cleanup.html")


@app.route("/admin/buyer-cleanup/run", methods=["GET", "POST"])
@login_required
def buyer_cleanup_run():
    """GET = preview (no writes). POST = apply.
    Renames/relocates rewrite the sale `buyer` (relocate also sets location and
    registers it under the parent); the delete purges matched records WITHOUT
    restoring FG stock (these were test adjustments — confirmed 2026-06-04)."""
    do_apply = request.method == "POST"
    sales  = _load_json(ORGANIC_SALES_PATH, [])
    buyers = _load_buyers()

    # normalized name -> exact stored buyer name, so targets keep real casing
    buyer_by_norm = {}
    for b in buyers:
        nm = (b.get("name") or "").strip()
        if nm:
            buyer_by_norm.setdefault(_bm_norm(nm), nm)

    actions = []
    delete_ids = set()
    buyers_changed = False

    for mig in _BUYER_MIGRATIONS:
        src_norm = _bm_norm(mig["source"])
        matched = [s for s in sales if _bm_norm(s.get("buyer")) == src_norm]
        distinct = sorted({(s.get("buyer") or "") for s in matched})
        units = sum(int(s.get("quantity") or 0) for s in matched)
        rep = {"action": mig["action"], "source": mig["source"],
               "matched": len(matched), "units": units,
               "distinct_strings": distinct, "warnings": [], "detail": ""}

        if mig["action"] == "rename":
            target = buyer_by_norm.get(_bm_norm(mig["target"]), mig["target"])
            rep["detail"] = "→ buyer “%s”" % target
            if _bm_norm(mig["target"]) not in buyer_by_norm:
                rep["warnings"].append(
                    "No active buyer named “%s” in buyers.json — records "
                    "will be relabelled but still show as their own card." % mig["target"])
            if do_apply:
                for s in matched:
                    s["buyer"] = target

        elif mig["action"] == "relocate":
            parent = buyer_by_norm.get(_bm_norm(mig["parent"]), mig["parent"])
            parent_exists = _bm_norm(mig["parent"]) in buyer_by_norm
            rep["detail"] = "→ buyer “%s”, location “%s”" % (parent, mig["location"])
            if not parent_exists:
                rep["warnings"].append(
                    "No parent buyer named “%s” in buyers.json — records will "
                    "be relabelled but the roll-up won't register a location." % mig["parent"])
            if do_apply:
                for s in matched:
                    s["buyer"] = parent
                    s["location_name"] = mig["location"]
                if parent_exists:
                    for b in buyers:
                        if b.get("name") == parent:
                            locs = b.setdefault("locations", [])
                            if not any(_bm_norm(l.get("name")) == _bm_norm(mig["location"])
                                       for l in locs):
                                locs.append({"id": str(len(locs)),
                                             "name": mig["location"], "address": ""})
                                buyers_changed = True
                            break

        elif mig["action"] == "delete":
            rep["detail"] = "purge records (FG stock untouched)"
            if do_apply:
                for s in matched:
                    delete_ids.add(id(s))
    ...
    if do_apply:
        if delete_ids:
            sales = [s for s in sales if id(s) not in delete_ids]
        _save_json(ORGANIC_SALES_PATH, sales)
        if buyers_changed:
            _save_buyers(buyers)

    return jsonify({"applied": do_apply, "actions": actions})
