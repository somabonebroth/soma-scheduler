...
# ─── CONTRACTS ───────────────────────────────────────────────────────────────
# Wholesale Supply Agreement between Soma and Ripe.
#
# Design notes (see CLAUDE.md for full context):
#   - Single source of truth lives in Soma. Ripe gets a read-only view.
#   - Each contract carries a frozen snapshot of prices + rules at signing
#     time. Future Soma changes don't retroactively alter what was agreed.
#   - Lifecycle is computed from dates + signatures + a manual supersede flag.
#     No status field on disk — _contract_status(c) is the only function that
#     decides what state a contract is in.
#   - PDF of the fully-signed contract is generated once both signatures are
#     captured and stored under CONTRACT_PDFS_DIR. The legal record.
#
# Status values returned by _contract_status:
#   draft               — snapshot not frozen yet, freely editable
#   pending_signatures  — snapshot frozen, one or both signatures missing
#   signed              — both signed, effective_date is in the future
#   active              — both signed, currently inside the effective window
#   expired             — past expiry_date
#   superseded          — manually marked, or a newer contract has taken over

CONTRACTS_PATH = os.path.join(INVENTORY_DIR, "contracts.json")
CONTRACT_PDFS_DIR = os.path.join(DATA_DIR, "contract_pdfs")
os.makedirs(CONTRACT_PDFS_DIR, exist_ok=True)


def _load_contracts():
    return _load_json(CONTRACTS_PATH, [])


def _save_contracts(contracts):
    _save_json(CONTRACTS_PATH, contracts)


def _contract_status(c):
    """Compute the lifecycle state of a contract record. Pure function of the
    record + today's date — never stored, always derived."""
    if c.get("manually_superseded"):
        return "superseded"
    if not c.get("snapshot"):
        return "draft"
    sigs = c.get("signatures") or {}
    if not (sigs.get("soma") and sigs.get("ripe")):
        return "pending_signatures"
    from datetime import date as _date
    today = _date.today()
    try:
        eff = _date.fromisoformat(c.get("effective_date", ""))
        exp = _date.fromisoformat(c.get("expiry_date", ""))
    except ValueError:
        # Missing or malformed dates — fall back to "signed" so the record is
        # at least visible. Should never happen if the UI validates correctly.
        return "signed"
    if today > exp:
        return "expired"
    if today < eff:
...
def _build_contract_snapshot(buyer_name="Ripe"):
    """Pull current SKU prices + rules into the shape we freeze on a contract.
    Used both when creating a draft (for pre-fill) and when freezing (the
    immutable record). The PRICE captured here is what binds — future catalogue
    edits don't alter signed contracts."""
    buyers = _load_buyers()
    buyer = next((b for b in buyers if b.get("name") == buyer_name), None)
    company = _load_company_info()
    skus = []
    if buyer:
        for sku in (buyer.get("skus") or []):
            ...
            skus.append({
                ...
                "case_price": round(unit_price * 12, 2) if unit_price is not None else None,
                "position":   sku.get("position", 999),
            })
        skus.sort(key=lambda s: (s["position"], s["format"], s["name"]))
    return {
        "skus": skus,
        "rules": {
            "ss_min_cases_delivery":    int(company.get("ss_min_cases_delivery") or 40),
        ...
        "payment_methods": [
            {"key": "etransfer", "label": "E-Transfer", "pct": -2.0,
             "note": "2% discount on subtotal. Bank confirmation required."},
            {"key": "cc_net14",  "label": "Credit Card — Net 14", "pct": 2.9,
             "note": "2.9% surcharge on subtotal. Invoice due 14 days from issue."},
        ],
        "delivery_locations": [
            {"key": "w1",         "label": "Coco Market",           "address": "2549 Yonge St, Toronto, ON M4P 2H9"},
            {"key": "w2",         "label": "Woodville",             "address": "155 Woodville Ave, East York, ON M4J 2R4"},
            {"key": "summerhill", "label": "Summerhill Warehouse",  "address": "244 Bartley Dr, North York, ON M4A 1G1"},
            {"key": "pickup",     "label": "Soma (Pickup)",         "address": "676 Pape Ave, Toronto, ON"},
        ],
        "units_per_case": 12,
    }


def _default_party_soma():
    """Pre-fill the Soma party block from company_info.json."""
    ci = _load_company_info()
    return {
        "name":          ci.get("name") or "Soma Bone Broth Co Ltd.",
        "address":       ci.get("address") or "",
        "city":          ci.get("city") or "",
        "registration":  ci.get("registration") or "",
        "contact_name":  "",
        "contact_title": "",
        "contact_email": ci.get("email") or "",
        "contact_phone": ci.get("phone") or "",
    }


def _default_party_ripe():
    """Pre-fill the Ripe party block from the Ripe buyer record."""
    buyers = _load_buyers()
    b = next((x for x in buyers if x.get("name") == "Ripe"), None) or {}
    return {
        "name":          b.get("legal_name") or b.get("name") or "Ripe Nutrition Inc.",
        "address":       b.get("address") or "",
        "city":          b.get("city") or "",
        "registration":  b.get("registration") or "",
        "contact_name":  b.get("contact_name") or "",
        "contact_title": b.get("contact_title") or "",
        "contact_email": b.get("contact_email") or b.get("email") or "",
        "contact_phone": b.get("contact_phone") or b.get("phone") or "",
    }
...
def contracts_page():
    ...
    enriched = [dict(c, status=_contract_status(c)) for c in contracts]
    return render_template("contracts.html", contracts=enriched)


@app.route("/contracts/<contract_id>")
@login_required
def contract_view_page(contract_id):
    contracts = _load_contracts()
    c = next((x for x in contracts if x.get("id") == contract_id), None)
    if not c:
        return render_template("contracts.html", contracts=[], not_found_id=contract_id), 404
    c = dict(c, status=_contract_status(c))
    return render_template("contract_view.html", contract=c)
...
def api_list_contracts():
    contracts = _load_contracts()
    return jsonify([dict(c, status=_contract_status(c)) for c in contracts])


@app.route("/api/contracts", methods=["POST"])
@login_required
def api_create_contract():
    """Create a new draft. Pre-fills parties, dates, and a tentative snapshot
    preview so the editor has something to show. The snapshot is NOT frozen
    here — it stays mutable until /freeze."""
    import uuid
    now = datetime.now(timezone.utc).isoformat()
    # Default July 1 to June 30 cycle, picking the year that makes sense for today
    from datetime import date as _date
    today = _date.today()
    if today.month >= 7:
        eff = _date(today.year, 7, 1)
    else:
        eff = _date(today.year, 7, 1)
        # If today is already in the first half of the year, the next cycle is this July
    exp = _date(eff.year + 1, 6, 30)
    contract = {
        "id": uuid.uuid4().hex[:12],
        "version": f"{eff.year}-{exp.year}",
        "effective_date": eff.isoformat(),
        "expiry_date":    exp.isoformat(),
        "manually_superseded": False,
        "created_at": now,
        "last_edited_at": now,
        "parties": {
            "soma": _default_party_soma(),
            "ripe": _default_party_ripe(),
        },
        "snapshot": None,        # populated by /freeze
        "snapshot_preview": _build_contract_snapshot("Ripe"),  # tentative, for editor display
        "frozen_at": None,
        "signatures": {"soma": None, "ripe": None},
        "signed_pdf_filename": None,
    }
    contracts = _load_contracts()
    contracts.append(contract)
    _save_contracts(contracts)
    return jsonify({"ok": True, "contract": dict(contract, status=_contract_status(contract))})


@app.route("/api/contracts/<contract_id>", methods=["PATCH"])
@login_required
def api_update_contract(contract_id):
    """Edit a draft. Once snapshot is frozen, only the manually_superseded flag
    can be changed."""
    data = request.get_json() or {}
    contracts = _load_contracts()
    ...
    if status == "draft":
        # Allow edits to dates, version, parties, and snapshot_preview values
        for field in ("effective_date", "expiry_date", "version"):
            if field in data:
                c[field] = (data[field] or "").strip()
        if isinstance(data.get("parties"), dict):
            for party_key in ("soma", "ripe"):
                if isinstance(data["parties"].get(party_key), dict):
                    c["parties"][party_key].update({
                        k: (v or "").strip() if isinstance(v, str) else v
                        for k, v in data["parties"][party_key].items()
                    })
        # Allow overriding the snapshot_preview (so user can edit prices/rules
        # before freezing). Defensive: only accept the keys we expect.
        if isinstance(data.get("snapshot_preview"), dict):
            preview = data["snapshot_preview"]
            if "skus" in preview and isinstance(preview["skus"], list):
                c["snapshot_preview"]["skus"] = preview["skus"]
            if "rules" in preview and isinstance(preview["rules"], dict):
                c["snapshot_preview"]["rules"] = preview["rules"]
            if "payment_methods" in preview and isinstance(preview["payment_methods"], list):
                c["snapshot_preview"]["payment_methods"] = preview["payment_methods"]
            if "delivery_locations" in preview and isinstance(preview["delivery_locations"], list):
                c["snapshot_preview"]["delivery_locations"] = preview["delivery_locations"]
    if "manually_superseded" in data:
        c["manually_superseded"] = bool(data["manually_superseded"])
    c["last_edited_at"] = datetime.now(timezone.utc).isoformat()
    contracts[idx] = c
    _save_contracts(contracts)
    return jsonify({"ok": True, "contract": dict(c, status=_contract_status(c))})


@app.route("/api/contracts/<contract_id>/freeze", methods=["POST"])
@login_required
def api_freeze_contract(contract_id):
    """Lock the snapshot. After this the contract is immutable except for
    capturing signatures and the manually_superseded flag."""
    contracts = _load_contracts()
    idx = next((i for i, c in enumerate(contracts) if c.get("id") == contract_id), None)
    if idx is None:
        return jsonify({"error": "Not found"}), 404
    c = contracts[idx]
    if _contract_status(c) != "draft":
        return jsonify({"error": f"Contract is {_contract_status(c)} — only drafts can be frozen"}), 409
    # The frozen snapshot is whatever's in snapshot_preview at this moment
    c["snapshot"] = c.get("snapshot_preview") or _build_contract_snapshot("Ripe")
    c["frozen_at"] = datetime.now(timezone.utc).isoformat()
    c["last_edited_at"] = c["frozen_at"]
    contracts[idx] = c
    _save_contracts(contracts)
    return jsonify({"ok": True, "contract": dict(c, status=_contract_status(c))})


@app.route("/api/contracts/<contract_id>/sign/<party>", methods=["POST"])
@login_required
def api_sign_contract(contract_id, party):
    if party not in ("soma", "ripe"):
        return jsonify({"error": "Party must be 'soma' or 'ripe'"}), 400
    data = request.get_json() or {}
    name  = (data.get("name") or "").strip()
    title = (data.get("title") or "").strip()
    if not name:
        return jsonify({"error": "Signatory name is required"}), 400
    contracts = _load_contracts()
    idx = next((i for i, c in enumerate(contracts) if c.get("id") == contract_id), None)
    if idx is None:
        return jsonify({"error": "Not found"}), 404
    c = contracts[idx]
    status = _contract_status(c)
    if status not in ("pending_signatures",):
        return jsonify({"error": f"Contract is {status} — cannot sign"}), 409
    sigs = c.setdefault("signatures", {"soma": None, "ripe": None})
    if sigs.get(party):
        return jsonify({"error": f"{party.title()} has already signed"}), 409
    sigs[party] = {
        "name": name,
        "title": title,
    ...
    c["last_edited_at"] = sigs[party]["signed_at"]
    # If both signatures now present, generate the signed PDF and supersede
    # any prior active contract automatically.
    if sigs.get("soma") and sigs.get("ripe"):
        # Auto-supersede the previous active/signed contract (only one in effect)
        for other in contracts:
            if other.get("id") == contract_id:
                continue
            if _contract_status(other) in ("signed", "active") and not other.get("manually_superseded"):
                other["manually_superseded"] = True
                other["last_edited_at"] = sigs[party]["signed_at"]
        # Generate PDF (non-fatal if it fails — record stays signed; PDF can be regenerated)
        try:
            from pdf_engine import generate_contract_pdf
            pdf_filename = f"contract_{c['id']}.pdf"
            pdf_path = os.path.join(CONTRACT_PDFS_DIR, pdf_filename)
            with open(pdf_path, "wb") as fh:
                generate_contract_pdf(fh, c, logo_path=_logo_path_or_none())
            c["signed_pdf_filename"] = pdf_filename
        except Exception as exc:
            logger.warning("Could not generate signed contract PDF: %s", exc)
    contracts[idx] = c
    _save_contracts(contracts)
    return jsonify({"ok": True, "contract": dict(c, status=_contract_status(c))})
...
@login_required
def contract_pdf_download(contract_id):
    contracts = _load_contracts()
    c = next((x for x in contracts if x.get("id") == contract_id), None)
    if not c:
        return "Not found", 404
    fname = c.get("signed_pdf_filename")
    if not fname:
        return "PDF not available — contract is not fully signed yet.", 404
    return send_from_directory(CONTRACT_PDFS_DIR, fname,
                               as_attachment=True,
                               download_name=f"Soma-Ripe-Contract-{c.get('version','')}.pdf")


def _logo_path_or_none():
    logo = os.path.join(app.static_folder, "logo.jpg")
    return logo if os.path.exists(logo) else None


@app.route("/api/internal/contract/active", methods=["GET"])
def internal_active_contract():
    """Returns the currently-active OR signed-but-not-yet-effective contract
    for Ripe to render in its read-only portal. If none exists, returns the
    most recent expired contract so the legal record stays visible. Returns
    404 only when no contract has ever been signed.
    Key-gated via X-Internal-Key.
    """
    internal_key = os.environ.get("INTERNAL_API_KEY", "")
    provided = request.headers.get("X-Internal-Key", "")
    import hmac as _hmac
    if not internal_key or not _hmac.compare_digest(provided.encode(), internal_key.encode()):
        return jsonify({"error": "Unauthorized"}), 401
    contracts = _load_contracts()
    # Prefer active, then signed (future-effective), then most recent expired
    by_status = {"active": [], "signed": [], "expired": []}
    for c in contracts:
        s = _contract_status(c)
        if s in by_status and not c.get("manually_superseded"):
            by_status[s].append(c)
    candidate = None
    for tier in ("active", "signed", "expired"):
        if by_status[tier]:
            candidate = max(by_status[tier], key=lambda x: x.get("effective_date", ""))
            break
    if not candidate:
        return jsonify({"error": "No signed contract exists"}), 404
    return jsonify(dict(candidate, status=_contract_status(candidate)))


@app.route("/api/internal/contract/<contract_id>/pdf", methods=["GET"])
def internal_contract_pdf(contract_id):
    """Internal PDF serve. Key-gated. Ripe portal pulls this through."""
    internal_key = os.environ.get("INTERNAL_API_KEY", "")
    provided = request.headers.get("X-Internal-Key", "")
    import hmac as _hmac
    if not internal_key or not _hmac.compare_digest(provided.encode(), internal_key.encode()):
    ...
    contracts = _load_contracts()
    c = next((x for x in contracts if x.get("id") == contract_id), None)
    if not c or not c.get("signed_pdf_filename"):
        return jsonify({"error": "Not found"}), 404
    return send_from_directory(CONTRACT_PDFS_DIR, c["signed_pdf_filename"])
