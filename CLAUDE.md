...
**Vessels — ONE list (2026-09-23).** `helpers.VESSELS` = K1, K2, K3, 115L, 115L-2; `helpers.HALF_VESSELS` = the two 115Ls, tested ONLY via `_is_half_vessel()` (half ingredients, half yield, half raw deduction). app.py, pdf_engine and the Create Schedule / Weekly / tablet templates (Jinja globals `VESSELS`/`HALF_VESSELS`) all read it — never re-add a local copy or a `== "115L"` test. Vessel names are stored in schedules, checklists, runs and `fg_id`s: never rename one with history, add a new name. Both schedule PDFs print a half vessel's card/yield halved (`recipe_data` on the days_map entry) — before this they printed the 115L at FULL.
...
**Ingredient lot rename (`/admin/rename-ingredients`, raw_materials.py, 2026-09-23).** Renaming a
recipe ingredient does NOT cascade to received lots (`item` is locked on a lot), so after the
recipes moved to "Organic X" names every "X" lot was stranded at zero — and Rebuild Raw Balances,
which replays past runs against CURRENT recipe names, would have marked every past batch short.
The tool (preview GET / apply POST on `/run`) renames EVERY lot of a name no active recipe or
...
data lives, who to call) is the highest-value of these. Also: Shopify/Clover importers have
timeouts but no retry. (The `vision_scan.py` timeout item is gone — the module was
deleted 2026-08-19; see below.)
